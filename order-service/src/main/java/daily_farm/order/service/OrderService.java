...
public class OrderService implements IOrderService {
	...
	public CreateOrderResponseDto createOrder(UUID farmsetId, UUID customerId) {
		...
		OrderFarmSet orderFarmSet = OrderFarmSet.builder()
				.farmSetId(farmsetId)
				.customerId(customerId)
				.farmerId(farmSetResponse.getFarmerId())
				.status(OrderStatus.WAITING_FOR_PAYMENT)
				.createdAt(LocalDateTime.now())
				.sumOfOrder(farmSetResponse.getPrice())
				.build();
		log.info("OrderService : Created new order");

		orderRepo.save(orderFarmSet);
		log.info("OrderService : Order saved to database ");
